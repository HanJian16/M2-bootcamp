...
function buildToDo(todo, index) {
  ...
  toDoText.innerHTML = todo.description;//<span>Hola we</span>
  
  // toDoText.setAttribute(`id`, index);

  let checkbox = document.createElement(`input`);//<input/>
  checkbox.setAttribute(`type`, `checkbox`);//<input type = `checkbox`/>
  checkbox.setAttribute(`id`, index);//<input type = `checkbox` id = 1/>
  checkbox.addEventListener(`click`, completeToDo);
  checkbox.setAttribute(`class`, `completeCheckbox`);

  if(todo.complete) checkbox.setAttribute(`checked`, true); //toDoText.setAttribute(`class`, `completeText`);
...
// La función 'buildToDos' debe crear un array de objetos toDo y de volverlo
...
function buildToDos(toDos) {
  // Tu código acá:
  let arrToDo = toDos.map((ele, index) => buildToDo(ele, index));
...
function displayToDos() {
  ...
  toDoBuild.forEach(toDo => toDoContainer.appendChild(toDo));
}
